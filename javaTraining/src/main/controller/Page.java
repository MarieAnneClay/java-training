...
public class Page {
  ...
  /* LIST */
  // COMPUTER
  public void printComputers(ArrayList<Computer> computers,
      ServiceComputer serviceComputer) {
    boolean flag = false;
    while (flag == false) {
      System.out.println("Veuillez saisir le numéro de la page souhaité ("
          + computerTotalPages + " pages) ou 0 pour sortir ");
      int nb = Integer.parseInt(sc.nextLine());
      if (nb > 0 && nb <= computerTotalPages) {
        ArrayList<Computer> computerSubest = serviceComputer
            .getComputerSubest((nb - 1) * PAGE_SIZE, PAGE_SIZE, computers);
        System.out.println("/***** AFFICHAGE DES ORDINATEUR  *****/");
        for (int i = 0; i < computerSubest.size(); i++) {
          Computer computer = computerSubest.get(i);
          System.out.println(
              "ID : " + computer.getId() + " nom : " + computer.getName());
        }
  ...
  public void printCompanies(ArrayList<Company> companies,
    ...
    while (flag == false) {
      System.out.println("Veuillez saisir le numéro de la page souhaité ("
          + companyTotalPages + " pages) ou 0 pour sortir ");
      int nb = Integer.parseInt(sc.nextLine());
      if (nb > 0 && nb <= companyTotalPages) {
        ArrayList<Company> companySubest = serviceCompany
            .getCompanySubest((nb - 1) * PAGE_SIZE, PAGE_SIZE, companies);
        System.out.println("/***** AFFICHAGE DES ENTREPRISES  *****/");
        for (int i = 0; i < companySubest.size(); i++) {
          Company company = companySubest.get(i);
          System.out.println(
              "ID : " + company.getId() + " nom : " + company.getName());
        }
